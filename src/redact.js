...
mammoth
  ...
  .then((result) => {
    ...
    for (const [type, dataMap] of Object.entries(evaluationMaps)) {
      let remaining = 0;

      dataMap.forEach((fakeValue, originalValue) => {
        if (res.includes(originalValue)) {
          remaining++;
        }
      });

      const detected = dataMap.size;
      const tp = detected - remaining;
